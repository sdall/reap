...
using Reap, Comonicon, JSON, GZip, CSV
...
This is a command line interface for the __Reap__ algorithm, which discovers non-redundant pattern-sets using the relaxed maximum entropy distribution.
...
_Example:_ `bin/reap.jl data.tsv labels.tsv --min-support=4 > result.json`
...
@main function reap(x, y = "";
    ...
    X, vocab = if any(e -> endswith(x, e), (".dat", ".dat.gz"))
        ...
        CSV.read(opts["x"], CSV.Tables.matrix; header=0, types=Bool) |> Reap._convert_dataset, nothing
    end
    Y = !isnothing(y) && !isempty(y) && isfile(y) ? read_labels(y) : nothing

    if measure_time && Threads.nthreads() > 1
        Reap.reap(X, Y; min_support=min_support, max_seconds=oftype(max_seconds, 0), max_discoveries=max_discoveries,
                  max_factor_size=max_factor_size, max_factor_width=max_factor_width)
    end

    t = @timedv p = Reap.reap(X, Y; min_support=min_support, max_seconds=max_seconds, max_discoveries=max_discoveries,
                              max_factor_size=max_factor_size, max_factor_width=max_factor_width)

    out = isnothing(vocab) ? e -> Set(e) : e -> Set(vocab[i] for i in e)
    S = p isa Vector ? [[out(e) for e in Reap.patterns(q)] for q in p] : [out(e) for e in Reap.patterns(p)]
